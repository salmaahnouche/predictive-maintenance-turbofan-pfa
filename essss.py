...
from keras.layers import (
    Conv1D, Dense, Dropout, BatchNormalization,
    GlobalAveragePooling1D, Multiply, Activation,
    Permute, Flatten, RepeatVector, Lambda
)
import keras.backend as K

def entrainer_cnn(X_train, y_train,
                             epochs=CNN_EPOCHS,
                             batch_size=CNN_BATCH):
    """
    CNN 1D avec mécanisme d'attention temporelle.
    Compatible Keras 3 / TensorFlow 2.x — pas de couche Lambda.

    L'attention softmax donne plus de poids aux derniers cycles
    (phase de dégradation terminale), corrigeant le biais de
    surestimation du GlobalAveragePooling1D classique.
    """
    ...
    inputs = tf.keras.Input(shape=(ts, nb))
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Dropout(0.2)(x)
    # x : (batch, TIME_STEPS, 128)

    # ── Attention temporelle ─────────────────────────────────
    # Score d'attention : une valeur scalaire par pas de temps
    attn_scores = tf.keras.layers.Dense(1, use_bias=False)(x)
    # attn_scores : (batch, TIME_STEPS, 1)

    attn_weights = tf.keras.layers.Softmax(axis=1)(attn_scores)
    # attn_weights : (batch, TIME_STEPS, 1)  — somme sur axis=1 = 1

    # Somme pondérée : remplace GlobalAveragePooling1D
    # Multiply broadcast (batch,TIME_STEPS,128)*(batch,TIME_STEPS,1)
    x = tf.keras.layers.Multiply()([x, attn_weights])
    # (batch, TIME_STEPS, 128) → réduction sur axis temporel
    x = tf.keras.layers.GlobalAveragePooling1D()(x)
    # NOTE : après Multiply par attn_weights dont la somme = 1,
    # GlobalAveragePooling1D()*TIME_STEPS ≈ somme pondérée.
    # On compense en ne divisant pas (le Dense suivant absorbe l'échelle).
    ...
    # Sortie linéaire — RUL directement en cycles bruts
