...
def asym_loss(y_true, y_pred):
    # Sécurité : on s'assure que les deux tenseurs ont le même type (float32)
    y_true = tf.cast(y_true, dtype=y_pred.dtype)
    
    # Calcul de l'écart
    d = y_pred - y_true
    
    # Si d > 0 (surestimation = danger), on multiplie l'erreur au carré par 5
    # Si d <= 0 (sous-estimation = prudence), on garde l'erreur au carré normale
    perte = tf.where(d > 0, 5.0 * tf.square(d), tf.square(d))
    
    # On retourne la moyenne de cette perte
...
def entrainer_cnn(X_train, y_train, epochs=30, batch_size=64):
    ...
    # Remplacez : model.compile(loss='mean_squared_error', optimizer='adam')
