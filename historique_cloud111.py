...
from sklearn.impute import SimpleImputer                          # ← from file 2
...
# 1. On entraîne le modèle avec ta fonction
model_cnn = entrainer_cnn(X_train, y_train)

# 2. On convertit le modèle Keras au format TFLite
converter = tf.lite.TFLiteConverter.from_keras_model(model_cnn)

# Optionnel mais recommandé : Quantification par défaut 
# (Sur un CNN, cela réduit la taille par 4 sans presque aucune perte de précision)
converter.optimizations = [tf.lite.Optimize.DEFAULT] 

tflite_model = converter.convert()

# 3. Sauvegarder le fichier .tflite
with open("modele_cnn.tflite", "wb") as f:
    f.write(tflite_model)
    
print("Conversion TFLite terminée !")

# Fonction pour convertir les octets en tableau C
...
# On ajoute alignas(16) pour la stabilité de l'ESP32
    # Et on remplace le saut de ligne simple par une virgule + saut de ligne (',\n')
# On définit le séparateur en dehors de la f-string pour éviter l'erreur de l'antislash
    separateur = ",\n  "
    f.write(f"alignas(16) const unsigned char cerveau_lstm_tflite[] PROGMEM = {{\n  "
            f"{separateur.join(hex_lines)}\n}};\n")
...
# ─────────────────────────────────────────
# Feature engineering (robuste aux NaN)   ← logique de sécurité du fichier 2
# ─────────────────────────────────────────
...
# ── SimpleImputer de sécurité (médiane) ← intégré depuis fichier 2 ──
...
for sensor, title, ax in sensor_plots:
    for fault_id, name in FAULT_NAMES.items():
        ...
        for unit in units_fault:
            ...
            # Nettoyage NaN par interpolation linéaire ← robustesse fichier 2
...
joblib.dump(imputer_clf, 'imputer_fault_classifier.pkl')   # ← nouveau (fichier 2)
joblib.dump(imputer_gmm, 'imputer_fault_gmm.pkl')           # ← nouveau (fichier 2)
